  ...
  const openWindow = (windowName) => {
    if (deviceState.isSmallScreen) {
      if (windows.length > 0) {
        ...
        setActive(windowName);  // Here, pass the window name as string
        return;
      }
    }
    ...
    const existingWindow = windows.find((win) => win.name === windowName);
    if (existingWindow) {
      setActive(existingWindow.name);  // Pass the window name as string
    } else {
      const newWindow = {
        id: windowName,  // Use string-based ID
        name: windowName,
      ...
      setWindows((prev) => [...prev, newWindow]);
      setActive(newWindow.id);  // Pass the window name as string
    }
  };
